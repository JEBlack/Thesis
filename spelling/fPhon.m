function rep = fPhon(word,phonRep,vRep)
...
%     cRep = readtable('cRep.xlsx');
    ...
    rep(1:12)=table2array(phonRep(:,word{1})); %consonant always first
    if(ismember(word{2},vowels))
        rep(25:36)=table2array(vRep(:,word{2})); %no initial consonant cluster, v
        rep(37:48)=table2array(phonRep(:,word{3})); %c
            if(length(word)==4)
                rep(49:60)=table2array(phonRep(:,word{4})); %final consonant cluster
            end
        
    else
        rep(13:24)=table2array(phonRep(:,word{2})); %initial consonant cluster, c
        rep(25:36)=table2array(vRep(:,word{3})); % v
        rep(37:48)=table2array(phonRep(:,word{4})); %c
        if(length(word)==5)
            rep(49:60)=table2array(phonRep(:,word{5})); %final cluster
        end
