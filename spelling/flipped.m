...
for hid = 1:5
    for lrate = 1:4
        for mome = 1:4
            for initial = 1:4
                for rep = 1:5
                    paramset= [paramset; lrates(lrate), alphas(mome), numHids(hid), initials(initial)];
                end
            end 
        end 
    end 
...
% Free Parameters
        ...
        numOut=156; % number of output nodes, orth
        numIn=60; % number of input nodes, phon
...
[setCorrect, numCorrect]= lettersCorrectDriver(W1,W2,biasW,wordsSize,words, phonRep, vRep);
